...
class Slider(models.Model):
    ...
    is_active = models.BooleanField(
        default=True,
        verbose_name='Включить слайдер на главную')
    ...
    class_css = models.CharField(
        ...
        max_length=255,
        blank=True)
    title_btn = models.CharField(
        'Название кнопки',
        default='Перейти',
        max_length=255)
    slug = models.SlugField(
        max_length=255,
        verbose_name='URL-на всякий',
        blank=True)
    image = models.ImageField(
        'Изображение большое', upload_to='sliders/')
    image_mobile = models.ImageField(
        'Изображение на мобильном',
        upload_to='sliders/',
        blank=True)
    ...
    def has_h1(self):
        if self.header_type == 'h1':
            return Slider.objects.filter(
                header_type='h1').exclude(pk=self.pk).exists()
        return Slider.objects.filter(
            header_type='h1').exists()

    def clean(self):
        if self.header_type == 'h1' and self.has_h1():
            raise ValidationError(
                'Заголовок H1 уже существует. '
                'На слайдерах может быть только один h1')
